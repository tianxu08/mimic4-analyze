...
from mimic4analysis.mimic4csv import *
...
if args.verbose:
    print('REMOVE MULTIPLE STAYS PER ADMIT:\n\stay_id: {}\n\hadm_id: {}\n\subject_id: {}'.format(stays.stay_id.unique().shape[0],
          stays.hadm_id.unique().shape[0], stays.subject_id.unique().shape[0]))
...
procedures = read_procedures_table(args.mimic4_path)
# patients = read_patients_table(args.mimic4_path)
...
# only keep the subjects with invasive ventilation
subjects = procedures.subject_id.unique()
...
break_up_procedures_by_subject(procedures, args.output_path, subjects=subjects)
items_to_keep = set(
    [int(itemid) for itemid in dataframe_from_csv(args.itemids_file)['ITEMID'].unique()]) if args.itemids_file else None

print("items_to_keep: ", items_to_keep)
